...
export default function AdminUsuariosPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-text-muted">
        <div className="w-12 h-12 border-4 border-menu-active border-t-transparent rounded-full animate-spin mb-4" />
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 font-sans animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      {error && (
        <div className="bg-rose-100 border border-rose-200 text-rose-700 font-medium px-4 py-3 rounded-xl relative mb-6" role="alert">
          {error}
        </div>
      )}
                  ...
                  <tr key={u.id} className="hover:bg-background/50 transition-colors group">
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-primary capitalize whitespace-nowrap">
                    ...
                    <td className="p-5 text-center">
                      {u.is_active ? (
                        <span className="text-[11px] font-bold text-green-700 bg-green-100 border border-green-200 px-3 py-1 rounded-full whitespace-nowrap">
                          Activo
                        </span>
                      ) : (
                        <span className="text-[11px] font-bold text-rose-700 bg-rose-100 border border-rose-200 px-3 py-1 rounded-full whitespace-nowrap">
                          Suspendido
                        </span>
                      )}
                    </td>
                    <td className="p-5 text-right space-x-2 whitespace-nowrap">
                      ...
                      {(u.rol === "padres" || u.rol === "parent") && (
                        <button
                          ...
                          className="text-[11px] font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-lg transition-colors inline-block"
                        >
                          Asignar Hijo
                        </button>
                      )}
      ...
      {isAssignChildModalOpen && selectedParent && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-surface rounded-3xl shadow-xl border border-border w-full max-w-md p-6 md:p-8 relative">
            <button onClick={() => setIsAssignChildModalOpen(false)} className="absolute top-6 right-6 text-text-muted hover:text-rose-500 font-bold">✕</button>
            <h2 className="text-2xl font-black text-primary mb-2">Asignar Hijo</h2>
            ...
            <form onSubmit={handleAsignarHijo} className="space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-text-muted uppercase tracking-wider mb-1">Alumno</label>
                <select
                  required
                  value={selectedChildId}
                  onChange={(e) => setSelectedChildId(e.target.value)}
                  className="w-full bg-background border border-border px-3 py-2 rounded-xl outline-none focus:border-menu-active font-medium"
                >
